      ...
      >
        <DialogTrigger asChild>
          <div className="relative flex-1 max-w-md cursor-pointer">
            ...
            <Input
              className="md:w-full rounded-md bg-background border h-9 pl-10 pr-0 sm:pr-7 text-sm shadow-sm overflow-ellipsis"
              placeholder="Search documentation..."
              type="search"
            />
            <div className="sm:flex hidden absolute top-1/2 -translate-y-1/2 right-2 text-xs font-medium font-mono items-center gap-0.5 bg-stone-200/65 p-1 rounded-sm">
              ...
              {filteredResults.map((item) => {
                ...
                return (
                  <DialogClose key={item.href} asChild>
                    <Anchor
                      className={cn(
                        "hover:bg-stone-100 w-full px-3 rounded-sm text-sm flex items-center gap-2.5",
                        paddingClass
                      )}
